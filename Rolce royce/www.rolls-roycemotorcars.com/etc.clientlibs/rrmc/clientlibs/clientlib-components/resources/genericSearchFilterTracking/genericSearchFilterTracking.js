// last build:2024-02-28T06:28:19.800Z
...
const PROJECT_SPECIFIC_DATA = {
    'customeEventName': 'TRON: generic search filter',
    'libraryName': 'generic_search_filter',
    'libraryVersion': '_v1.1.27',
    'prefix': 'nsc > ',
...
const config = {
  ...
  defaults: function (scope) {
    ...
    scope.component = {
      ...
      componentInfo: {
        componentID: `TRON_${scope._TronVersion}__${scope._componentTrackingVersion}`, // hard coded for all events
        componentName: 'generic search filter',
      },
      attributes: {
    ...
    scope.baseEvent = {
      eventInfo: {
        ...
        eventName: "search", //value might be overwritten by value specified in event below
        eventPoints: 0, //value might be overwritten by value specified in event below
        type: "", //value might be overwritten by value specified in event below
      },
      category: {
...
// ****************************************
    scope.errorMandatoryData = ['componentName','filterObject'];// only add non-error parameters that are mandatory also at error event eg. componentName
    scope.baseMandatoryData = ['componentName'] // all product data that is available in the front end must be provided in dynamic parameters as per description
    scope.baseOptionalData = ['componentDEVagency','componentPlatform','componentVersion',];
...
// **************************
    scope.allowedComponentNames = ['model context filter', 'boutique', 'fs widget'];
...
// *** end allowedValuesComponents.js ***
    // *** start allowedValuesSearch.js ***

// **************************
// * ALLOWED VALUES: SEARCH *
// **************************

scope.allowedFilterObjects = ['accessory','content', 'dealer', 'finance', 'vehicle', 'service', 'brochure'];
scope.allowedSearchState = ['preselected'];

// *** end allowedValuesSearch.js ***
  },
  ...
  events: function (scope) {
    ...
    scope.events = Object.assign(scope.events, {
    //START OF EVENTS
    // *** start configEventsSearch.txt ***

// ******************
// * EVENTS: SEARCH *
// ******************

[(functionPrefix + "FilterSort").replace(/^(.)(.*)$/, (match, firstChar, restOfString) => firstChar.toLowerCase() + restOfString)]: {
    event: {
        eventInfo: Object.assign({}, JSON.parse(JSON.stringify(scope.baseEvent.eventInfo)), {
            effect: "search results sorted by",
            eventAction: "sort",
    ...
    mandatoryData: scope.baseMandatoryData.concat(['sortType',]).filter(v => !['search[n].searchLabel', 'search[n].searchTerm','searchResults', 'eventSubCat1'].includes(v)),
    optionalData: scope.baseOptionalData.filter(v => !['searchResults','eventSubCat1'].includes(v)).concat(scope.productMandatoryData, scope.productOptionalData, ['eventCause','search[n].searchLabel', 'search[n].searchTerm','searchResults','filterObject','search[n].searchUnit', 'search[n].searchState', 'eventPoints',]),
    description: `Trigger this whenever the user sorts the search results.
        <br><ul>
            ...
            <li><b>'search[n].searchUnit'</b> is used to capture the unit of a numeric search term eg. "km", "mi", "â‚¬", "$", etc.</li>
...
[(functionPrefix + "FilterRemoveAll").replace(/^(.)(.*)$/, (match, firstChar, restOfString) => firstChar.toLowerCase() + restOfString)]: {
    event: {
        eventInfo: Object.assign({}, JSON.parse(JSON.stringify(scope.baseEvent.eventInfo)), {
            effect: "filters removed all",
            eventName: "search",
            eventAction: "internal click",
            cause: "default"
        })
    },
    mandatoryData: scope.baseMandatoryData.concat(['filterObject','search[n].searchLabel','search[n].searchTerm']),
    optionalData: scope.baseOptionalData.filter(v => !['searchResults'].includes(v)).concat(scope.productMandatoryData, scope.productOptionalData, ['searchResults','eventAction','search[n].searchState','search[n].searchUnit', 'eventPoints' ]),
...
[(functionPrefix + "FilterRemoved").replace(/^(.)(.*)$/, (match, firstChar, restOfString) => firstChar.toLowerCase() + restOfString)]: {
    event: {
        eventInfo: Object.assign({}, JSON.parse(JSON.stringify(scope.baseEvent.eventInfo)), {
...
[(functionPrefix + "FilterSelected").replace(/^(.)(.*)$/, (match, firstChar, restOfString) => firstChar.toLowerCase() + restOfString)]: {
    event: {
        eventInfo: Object.assign({}, JSON.parse(JSON.stringify(scope.baseEvent.eventInfo)), {
            effect: "filter selected",
            eventName: "search",
            eventAction: "internal click",
            cause: "default"
        })
    ...
    mandatoryData: scope.baseMandatoryData.concat(['filterObject','search[n].searchLabel','search[n].searchTerm']),
    optionalData: scope.baseOptionalData.filter(v => !['searchResults'].includes(v)).concat(scope.productMandatoryData, scope.productOptionalData, ['searchResults','eventAction','search[n].searchState','search[n].searchUnit', 'eventPoints','eventCause' ]),
    description: `Trigger this event every time filters are selected.
...
if (typeof mandatoryData != 'undefined' && typeof mandatoryData['filterObject'] != 'undefined') {
    let filterObjects = mandatoryData['filterObject'];
    if (!this.allowedFilterObjects.includes(filterObjects)) {
        if (this.events[eventName].mandatoryData.indexOf('filterObject') > -1) {
            let determineErrorCase = (typeof mandatoryData['filterObject'] == "string" && mandatoryData['filterObject'].trim().length == 0) ? 'empty' : 'incorrect';
            this._throwError('m', 'filterObject', determineErrorCase, this.allowedFilterObjects);
            return;
        } else {
            let determineErrorCase = (typeof mandatoryData['filterObject'] == "string" && mandatoryData['filterObject'].trim().length == 0) ? 'incorrect-o' : 'incorrect-o';
            this._throwError('o', 'filterObject', determineErrorCase, this.allowedFilterObjects);
        }
        //this._throwError('m','filterObject','incorrect',this.allowedFilterObjects);
        //return;
    }
    mandatoryData['filterObject'] = filterObjects;
}

if (typeof mandatoryData != "undefined" && this.message.component[0].category.subCategory1 != 'compare' && this.message.component[0].category.primaryCategory != 'stock locator') {
    // if (typeof mandatoryData['filterObject'] != "undefined") {
    //     this.message.component[0].category.subCategory1 = this.component.category.subCategory1 + " " + mandatoryData['filterObject'] || this.component.category.subCategory1;
    // }
    if (typeof mandatoryData['filterObject'] != "undefined") {
        this.message.component[0].category.subCategory1 = gFFWNV([
            () => (this.message.component[0].category.subCategory1 || this.component.category.subCategory1) + " " + mandatoryData['filterObject'] || this.component.category.subCategory1
...
if (typeof mandatoryData != 'undefined' && typeof mandatoryData['search'] != 'undefined' && mandatoryData['search'].length > 0) {
    var flagVariable = false;
    for (var i in mandatoryData['search']) {
        if (typeof mandatoryData['search'][i].searchState != "undefined") {
            if (!this.allowedSearchState.includes(mandatoryData['search'][i].searchState)) {
                flagVariable = true;
                mandatoryData['search'][i].searchState = '';
            }
        }
    }
    if (flagVariable) {
        if (this.events[eventName].mandatoryData.indexOf('search[n].searchState') > -1) {
            this._throwError('m', 'search[n].searchState', 'incorrect', this.allowedSearchState);
            return;
        } else {
            this._throwError('o', 'search[n].searchState', 'incorrect-o', this.allowedSearchState);
        }
    }
}
...
//mapping the parameter "sortType"
if (typeof mandatoryData['sortType'] != 'undefined' && this.message.event[0].eventInfo.eventAction == 'sort') {
    //this.message.event[0].eventInfo.effect = this.message.event[0].eventInfo.effect + " " + mandatoryData['sortType'];
    this.message.event[0].eventInfo.effect = gFFWNV([
        () => this.message.event[0].eventInfo.effect + " " + mandatoryData['sortType']
    ]);
}

if (typeof mandatoryData['sortType'] != 'undefined' && this.message.event[0].eventInfo.eventAction == 'sort') {
    if (typeof mandatoryData['searchResults'] != 'undefined') {
...
if (typeof mandatoryData['filterObject'] != 'undefined' && this.message.event[0].eventInfo.eventName == 'search') {
    this.message.event[0].eventInfo.type = mandatoryData['filterObject'];
    this.message.event[0].eventInfo.effect = mandatoryData['filterObject'] + " " + this.message.event[0].eventInfo.effect;
}
//mapping searchResults
if (typeof mandatoryData['searchResults'] != 'undefined' && this.message.event[0].eventInfo.eventName == 'search') {
    this.message.event[0].category.subCategory1 = mandatoryData['searchResults'];
}
...
// search array
this.message.event[0].attributes.search = [];
if (mandatoryData['search'] && typeof mandatoryData['search'] != 'undefined') {
    mandatoryData['search'].forEach((searchAttribute) => {

        let searchAttributesElement = {};

        searchAttributesElement['label'] = gFFWNV([
            () => searchAttribute.searchLabel
        ]);

        if (typeof searchAttribute.searchTerm != "undefined") {
            searchAttributesElement['term'] = searchAttribute.searchTerm;
        }
        searchAttributesElement['term'] = gFFWNV([
            () => searchAttribute.searchTerm
        ]);

        searchAttributesElement['state'] = gFFWNV([
            () => searchAttribute.searchState,
            () => 'user selected'
        ]);

        searchAttributesElement['unit'] = gFFWNV([
            () => searchAttribute.searchUnit
        ]);

        this.message.event[0].attributes.search.push(searchAttributesElement);
    });
}
// *** end eventHandlerSearch.js ***
