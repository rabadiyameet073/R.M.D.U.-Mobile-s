// last build:2023-10-26T05:07:35.314Z
...
const PROJECT_SPECIFIC_DATA = {
    'customeEventName': 'TRON: enhanced activity map',
    'libraryName': 'enhanced_activity_map',
    'libraryVersion': '_v3.1.23',
    'prefix': 'nsc > ',
  ...
  defaults: function (scope) {
    ...
    scope.component = {
      category: {
        ...
        subCategory1: 'navigation' // hard coded for all events
      },
      componentInfo: {
        componentName: 'enhanced activity map', // hard coded for all events
        componentID: 'enhanced activity map' // hard coded for all events
      },
      attributes: {
...
// ****************************************
    scope.errorMandatoryData = [];// only add non-error parameters that are mandatory also at error event eg. componentName
    ...
    scope.baseOptionalData = ['componentDEVagency','componentPlatform','vehicle[n].productID', 'options[n].productID'];
...
// *** end allowedValuesComponents.js ***
  ...
  events: function (scope) {
    ...
    /*activityMapClick: {
        ...
        mandatoryData: scope.baseMandatoryData,
        optionalData: scope.baseOptionalData,
        description: `Trigger this event whenever the user clicks on a navigation item that does not navigate the user to a new page and hence triggers a server call<br>
    <u><i>Example: </i></u><br>
        tron.activityMapClick({<br>
        <div style="text-indent:10px;">componentVersion:"last build:2022-01-13T19:22:56.138Z", // mandatory </div>
        <div style="text-indent:10px;">});<br>
    <u><i>expected values </i></u><br> 
    <ul>
        <li>"componentVersion" = everytime the code version changes this value needs to be updated. The format is up to the developer. Samples are timestamp, or v1.0, etc</li>
    </ul>`
    },*/
    //END OF EVENTS
}) 
